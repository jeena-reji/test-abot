...
def execute_feature():
    ...
    # Many ABot APIs return executionId or timestamp → use that
...
def get_artifact_by_execution(exec_id):
    res = requests.get(ARTIFACT_BY_EXEC_URL, headers=headers, params={"execution": exec_id})
    res.raise_for_status()
    data = res.json().get("data", {})
    folder_name = data.get("artifact_name")
    print(f"📁 Artifact for this execution: {folder_name}")
    return folder_name
