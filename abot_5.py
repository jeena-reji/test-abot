...
ARTIFACT_DETAILS_URL = f"{ABOT_URL}/abot/api/v5/artifacts/execFeatureDetails"
...
def get_artifact_by_execution(exec_id):
    ...
    if res.status_code == 404 or not res.json().get("data"):
        ...
        return None
    res.raise_for_status()
    return res.json().get("data", {}).get("folderName")


def get_latest_artifact():
    """Fallback to latest artifact if execution artifact not found."""
    res = requests.get(LATEST_ARTIFACT_URL, headers=headers, timeout=30)
    if res.status_code != 200:
        print("⚠️ Could not fetch latest artifact.")
        return None
    data = res.json().get("data", {})
    folder_name = data.get("latest_artifact_timestamp")
    gui_urls = data.get("artifact_urls", [])
    if folder_name:
        print(f"📦 Latest artifact: {folder_name}")
        if gui_urls:
            print(f"🔗 GUI URL: {gui_urls[0]}")
        return folder_name
    print("⚠️ No latest artifact available.")
    return None


def get_artifact_details(foldername, feature_file):
    """Fetch detailed artifact info for a given folder and feature."""
    params = {"foldername": foldername, "featureId": feature_file}
    res = requests.get(ARTIFACT_DETAILS_URL, headers=headers, params=params, timeout=30)
    if res.status_code != 200:
        print(f"⚠️ Could not fetch artifact details for {foldername}/{feature_file}")
        return None
    return res.json()
...
if __name__ == "__main__":
    ...
    if folder:
        ...
        # Fetch detailed artifact info for main feature
        feature_file = FEATURE_TAG.split("@")[0] + ".feature"
        details = get_artifact_details(folder, feature_file)
        if details:
            print("\n📊 Artifact Detailed Info:")
